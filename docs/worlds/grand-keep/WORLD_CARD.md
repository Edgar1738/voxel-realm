...
- **Main status:** Showcase package shipped on `main` (manifest + VRW1 binary); M3 polish re-bake after hotel/solar fix.
- **Source assets:** Generator preset `grand-keep` + baked neighborhood package (~196 chunks, ~11 MB) + full procedural site beyond bake.
- **Registry evidence:** `world-manifest.json`, `public/worlds/grand-keep.vrw`, `public/worlds/previews/grand-keep.jpg`
