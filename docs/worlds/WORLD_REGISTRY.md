...
| `ember-spire`           | Ember Spire             | IMPLEMENTED WORLD    | main (origin: `grok/ember-spire`)                           | none                                              | Grok                             | shipped via PR #76          | high       | Formerly `ashen-reach`; source reconciled from `6034734`, registered as a preset, tested, benchmarked, and packaged as VRW1 in `4d4868a`                                                      |
| `grand-keep`            | The Grand Keep          | IMPLEMENTED WORLD    | `grok/grand-keep` (origin)                                  | `.claude/worktrees/grok+grand-keep`               | Grok                             | shipped on `main`           | high       | In `world-manifest.json` + `public/worlds/grand-keep.vrw`. Generator preset + baked neighborhood. `WORLD_HEIGHT` 512 / SAVE_VERSION 2.                                                        |
...
| `stonehaven`            | Stonehaven              | ACTIVE PROTOTYPE     | main (legacy: `experiment/project-stonehaven`)              | legacy dirty worktree; preserve captures          | Codex                            | M8 preset-in-main           | high       | Generator/site implementation through M8 is tested on main; still not a packaged manifest world. Legacy M2/M3 captures must be preserved before worktree cleanup. Source name `stonehaven-world` |
