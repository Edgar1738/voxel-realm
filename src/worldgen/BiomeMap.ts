...
export class BiomeMap implements BiomeSource {
  ...
  // Key: template-literal `${worldX},${worldZ}` — collision-free for all integer coords
  // with no range restriction. The wholesale-clear cap remains: bursty chunk generation
  // reuses points within a burst, so a simple cap bounds memory without hurting locality much.
