...
import { GRASS, DIRT, STONE, SAND, GRAVEL, DEEPSLATE, TERRACOTTA, MUD } from '../blocks/blocks';
 ...
 */
/**
 * Official identity (Milestone 2+): permanent Voxel Realm world â€” not an experiment name.
...
export const ASHEN = {
   ...
   * rises from rock, not from open water â€” readable silhouette from Emberhold and the rim.
   */
  ...
  outerY: 78,
  /**
   * Arrival approach north of the district: enclosed pass â†’ Crater Gate â†’ first reveal.
 ...
 * heights; the site overlay paves the same line. Order: plaza â†’ bridge â†’ south shore â†’ observatory.
 */
...
/** Radial distance from the caldera heart, with slight elliptical squash (eastâ€“west wider). */
...
/** Superellipse falloff inside the lake: 1 at center â†’ 0 at rim. */
...
/** Soft radial mask for a circular feature (1 at center â†’ 0 at r). */
...
function samplersFor(seed: WorldSeed): Samplers {
  ...
  if (!s) {
    const mk = (salt: number): NoiseFunction2D => createNoise2D(mulberry32((seed ^ salt) >>> 0));
...
function ashenHeight(noise: Samplers, wx: number, wz: number): number {
  ...
  const rimCrestStart = ASHEN.rimInner + 8; // ~96 â€” start of high crest plateau
  let h: number;
  if (d < terraceEnd) {
  ...
  // â”€â”€ Milestone 2: large authored geological landmarks (break up procedural rim noise) â”€â”€
  // SE basalt cliff horn â€” a single massive silhouette for orientation.
  {
  ...
  // NW monastery shelf â€” walkable flat for the cliff complex.
  {
  ...
  // NE terraced rim shelf.
  {
    const m = radialMask(
      wx,
      wz,
      ASHEN.terraceShelf.cx,
      ASHEN.terraceShelf.cz,
      ASHEN.terraceShelf.r,
    );
    if (m > 0.25) h = lerp(h, ASHEN.terraceShelf.y, Math.min(1, m * 1.3));
  }
  // SW ash ravine â€” deep cut for a recognizable gap in the wall.
  {
  ...
  // Arrival pass north of the district: flat floor + raised walls (enclosed approach â†’ reveal).
  {
 ...
 * No caves â€” solid ground so architecture never floats over voids.
 */
