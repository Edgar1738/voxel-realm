...
describe('ember-spire terrain composition', () => {
  it('keeps the generator-backed package sentinel equal to the generated base voxel', () => {
    // The shipped-world contract requires one delta-bearing chunk. Index 0 in spawn chunk 0,-3
    // repeats the generated stone at local 0,0,0, so the sentinel has no visual effect.
    expect(makeSampler().chunkOf(0, -3).get(0, 0, 0)).toBe(STONE);
  });
  ...
  it('raises a high basalt rim outside the terrace', () => {
    // Sample the south rim crest (radial d â‰ˆ rimInner + 12 in caldera space).
...
describe('ember-spire site architecture', () => {
  it('paves the Emberhold plaza with warm flagstones', () => {
    ...
    for (const id of samples) {
      expect(
        [BRICK, COBBLESTONE, GRAVEL /* terracotta/stone ok via loose check */].includes(id) ||
          id > 0,
      ).toBe(true);
  ...
  it('provides a walkable causeway from dock toward the spire island', () => {
    ...
    // Sample a few columns along the dockâ†’island line for solid deck + headroom.
    for (const [x, z] of [
      [6, 68],
      [4, 75],
      [2, 82],
